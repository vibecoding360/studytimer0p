...
import { Upload, FileText, Sparkles, CheckCircle2, X, File } from "lucide-react";
...
export default function UploadZone({ onFile, onTextChange, text, parsing, parsed, onParse, uploadedFileName, onClearFile }: Props) {
  ...
  const hasContent = text.trim().length > 0 || !!uploadedFileName;

  return (
    <div className="space-y-4">
      {/* Drop zone */}
      {uploadedFileName ? (
        <div className="relative border-2 border-primary/40 rounded-2xl p-6 bg-primary/5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">Ready to parse</p>
            </div>
            {onClearFile && (
              <button
                onClick={onClearFile}
                className="w-7 h-7 rounded-full bg-secondary/50 flex items-center justify-center hover:bg-destructive/20 transition-colors"
              >
        ...
        <div
          ...
          onClick={() => {
            ...
            input.type = "file";
            input.accept = ".txt,.pdf,.png,.jpg,.jpeg,.webp";
            input.onchange = (e) => {
      ...
      {/* Text area - show when no file uploaded */}
      {!uploadedFileName && (
